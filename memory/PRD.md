...
### 2026-02 — Prewarm translation cache + order-edit price recompute bugfix
- [x] **P1: First-visit pre-translate** — when a non-AZ user lands on any staff dashboard, the `useAutoTranslatePage` hook now fires a single **pre-warm** batch request BEFORE the MutationObserver has to react. `dashboardStrings.js` exports a curated list of 93 common AZ labels (nav, tile names, stat captions, status words, buttons, table headers) that is translated & written to localStorage in one shot. A per-language `prewarmed` flag in localStorage prevents re-firing. Effect: English/Russian/Turkish users now see the admin panel in their language after ~1-2s instead of a ~4s flash of AZ. Verified: `Prewarmed: 1, Cache size: 93`, body-text audit shows `"Admin Panel / Manage restaurant operations / TODAY'S REVENUE / ACTIVE ORDER / PENDING DELIVERY / Tables / Reservations / Menu / ..."`.
- [x] **P0: Order-edit price recompute bug** (root cause of reported 132₼ total while line item showed `x4 = 165₼`) — `PUT /api/orders/{id}` used to blindly persist client-sent `items` + `total_amount`. When admin edited quantity from 5→4, the frontend sent `discounted_price=165` (stale from qty=5) with `total_amount=132`, so MongoDB ended up inconsistent. Fixed by server-side recompute: backend now ignores client's `total_amount`/`discounted_price` and re-resolves `price` + `discount_percentage` from the canonical `menu_items` collection, multiplies by the (trusted-only) quantity and writes back a consistent `items`/`subtotal`/`total_amount` triad. Also fixes a security hole — client can no longer manipulate totals. Verified: sent stale `discounted_price=165` + `total_amount=999`, server responded with `total_amount=132.0` and DB now stores `discounted_price=132, subtotal=132, total_amount=132`.
